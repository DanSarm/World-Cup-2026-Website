import { flagImageUrlForSize, flagImageUrl2xForSize, type FlagSize } from "@/lib/flags";
...
export function Flag({ fifaCode, size = "md", className = "", title }: FlagProps) {
  if (!fifaCode) {
    return (
      <span
  ...
  const src = flagImageUrlForSize(fifaCode, size);
  if (!src) {
    return (
      <span
  ...
  return (
    ...
    <img
      src={src}
      srcSet={`${flagImageUrl2xForSize(fifaCode, size)} 2x`}
      alt={title ? `${title} flag` : `${fifaCode} flag`}
      title={title}
      width={size === "xs" ? 20 : size === "sm" ? 28 : size === "md" ? 40 : size === "lg" ? 56 : 80}
      height={size === "xs" ? 15 : size === "sm" ? 21 : size === "md" ? 30 : size === "lg" ? 42 : 60}
